...
export function DataTable<TData, TValue>({
  ...
  return (<>
    <div className="h-[60px] flex items-center w-[100vw] bg-blue-400">
      <CardTitle className="h-full  flex items-center flex-1 justify-center text-white">Running Projects </CardTitle>
    </div>
          ...
          {table.getHeaderGroups().map((headerGroup) => (
            <TableRow key={headerGroup.id}>
              {headerGroup.headers.map((header,idx) => {
                return (
                  <TableHead className={`${idx===0||idx===4?"test:hidden":""} ${idx===0?"hidden":""}`} key={header.id}>
            ...
            table.getRowModel().rows.map((row,idx) => (
              ...
              className={`${row.getValue("instituteId")!=JSON.stringify(id)?"hidden":""}`}               >
                {row.getVisibleCells().map((cell,idx) => (
                  <TableCell key={cell.id} className={`${idx===0||idx===4?"test:hidden":""} ${idx===0?"hidden":""}`}>
